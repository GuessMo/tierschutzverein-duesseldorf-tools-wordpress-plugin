...
function tsvd_halter_template_texts( $event, $case, $tier, $links, $permalink ) {
	...
	$private = array(
		...
		'paused'   => array( sprintf( __( 'Anzeige für %s pausiert', 'tsv-tools' ), $tier ), sprintf( __( "die Anzeige für %1\$s ist pausiert und nicht mehr zu sehen.\n\nWieder anzeigen (wir prüfen kurz und schalten frei): %2\$s", 'tsv-tools' ), $tier, $links['active'] ) ),
		'invite'   => array( sprintf( __( 'Deine Anzeige für %s verwalten', 'tsv-tools' ), $tier ), sprintf( __( "ab sofort kannst Du den Stand Deiner Anzeige für %1\$s selbst melden, ohne Anmeldung:\n%2\$s\n\nAnfragen leiten wir Dir per Mail weiter, zusammen mit den Kontaktdaten der anfragenden Person. Antworte ihr dann direkt. Deine eigenen Kontaktdaten geben wir nicht heraus.", 'tsv-tools' ), $tier, $links['owner'] ) ),
	...
	$missing = array(
		...
		'invite'    => array( sprintf( __( 'Deine Vermisstmeldung für %s verwalten', 'tsv-tools' ), $tier ), sprintf( __( "ab sofort kannst Du den Stand Deiner Vermisstmeldung für %1\$s selbst melden, ohne Anmeldung:\n%2\$s\n\nSichtungen leiten wir Dir per Mail weiter, zusammen mit den Kontaktdaten der Person, die Dein Tier gesehen hat. Melde Dich dann direkt bei ihr. Deine eigenen Kontaktdaten geben wir nicht heraus.", 'tsv-tools' ), $tier, $links['owner'] ) ),
